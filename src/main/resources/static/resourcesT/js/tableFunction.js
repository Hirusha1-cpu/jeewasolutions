...
const fillDataIntoPurcahsechkTable = (tableId, dataList, propertyList, purchaseButtonFunction, deleteButtonFunction,sendButtonFunction,checkBoxButton,
    buttonVisibility = true, privilegeOb = null) => {
    //create variable for store body
    const tableBody = tableId.children[1];
    tableBody.innerHTML = '';
    ...
    dataList.forEach((item, index) => {
        ...
        inputchk.className = "p-2 form-check-input"
        inputchk.id = "chk" + item.id


        inputchk.onchange = function () {
            if (this.checked) {
                console.log("ss clicked", item.name);
                checkBoxButton(item, index)
                this.checked = false
            }
