...
public interface CustomerDao extends JpaRepository<Customer, Integer>{
    ...
    // @Query(value = "SELECT * FROM jeewacomputersproject.customer where name = ?1", nativeQuery = true)
    // public Customer getCustomerByName(String name);
    ...
    @Query(value = "select ct FROM CustomerType ct where ct.customertypes = 'gold'")
    public CustomerType getPremiumBuyRounds();

    @Query(value = "select ct FROM CustomerType ct where ct.customertypes = 'silver'")
    public CustomerType getFirstStageBuyRounds();

    @Query(value = "select ct FROM CustomerType ct where ct.customertypes = 'bronze'")
    ...
    @Query("SELECT ct FROM CustomerType ct WHERE ct.id IN (SELECT c.customerType.id FROM Customer c WHERE c.name = ?1 and c.phone = ?2)")
    public CustomerType getCustomerTypeByPhoneandName(String name,String phone);
