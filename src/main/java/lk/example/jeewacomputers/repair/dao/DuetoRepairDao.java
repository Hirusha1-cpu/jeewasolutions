...
public interface DuetoRepairDao extends JpaRepository<DuetoRepair, Integer>{
    ...
    @Query(value = "select d from DuetoRepair d where d.repairtype = ?1 and d.statusofrepair = 'pending diagnosis ' and d.statusofrepair != 'Deleted' ")
    public List<DuetoRepair> getDueRepairByStatus(String repairtype);

    @Query(value = "select d from DuetoRepair d where d.statusofrepair != 'Completed' and d.statusofrepair != 'Paid' and d.statusofrepair != 'Deleted'")
    ...
    @Query(value = "select d from DuetoRepair d where d.statusofrepair = 'Approved' and d.statusofrepair != 'Deleted'")
    public List<DuetoRepair> getDueRepairByStatusApprove();

        ///////
    //ORDER BY RAND() LIMIT 1
    @Query(value = "SELECT * FROM jeewacomputersproject.due_to_repairitem where repairtype = 'shop item' and itemname = ?1 ORDER BY RAND() LIMIT 1", nativeQuery = true)
    public DuetoRepair getDueRepairByStatusShopItem(String itemname);
    ////////

    @Query(value = "select d from DuetoRepair d where  d.statusofrepair ='Diagnoesed' and d.statusofrepair != 'Deleted' ")
    public List<DuetoRepair> getDueRepairByStatusForProcessing();

    //and d.statusofrepair !='Approved' ain kara
    @Query(value = "select d from DuetoRepair d where  d.statusofrepair !='pending diagnosis' and d.statusofrepair !='Return To Company' and d.statusofrepair !='Approved' and d.statusofrepair !='Paid' and d.statusofrepair !='Deleted'")
