...
public interface PrivilegeDao extends JpaRepository<Privilege, Integer> {
  ...
  @Query(value = "SELECT bit_or(p.sel) as sel , bit_or(p.inst) as inst, bit_or(p.upd) as upd, bit_or(p.del) as del FROM jeewacomputersproject.privilege as p where p.role_id in (select uhr.role_id from jeewacomputersproject.user_has_role as uhr where uhr.user_id in (select u.id from jeewacomputersproject.user as u where u.username=?1)) and p.module_id in (select m.id from jeewacomputersproject.module as m where m.name=?2);", nativeQuery = true)
