...
public interface IncomePaymentDao extends JpaRepository<IncomePayment, Integer> {
    ...
    @Query(value ="SELECT i.invoiceno,i.date,s.customer_id,s.total FROM jeewacomputersproject.incomepayment as i inner join  jeewacomputersproject.sales as s on i.sales_id = s.id;",nativeQuery = true)
